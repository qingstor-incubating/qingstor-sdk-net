...
namespace QingStor_SDK_NET.Request
{
    // Paramter Type
    enum EParamType
    {
...
#elif NET35
                            if (!CGlobalSet.SetSubResources.Contains(Item.Key.ToString()))
                            {
                                continue;
                            }
#endif
...
#elif NET35
            MethodInfo Method = HttpRequest.Headers.GetType().GetMethod("AddWithoutValidate", BindingFlags.Instance | BindingFlags.NonPublic);
            Method.Invoke(HttpRequest.Headers, new[] { "Date", DateTime.Now.ToUniversalTime().ToString("r") });
#endif
            // Add Header Params
...
#elif NET35 || NET40
                    Object ParamValue = ProInfo.GetValue(InputParam, null);
#endif
                    // header
